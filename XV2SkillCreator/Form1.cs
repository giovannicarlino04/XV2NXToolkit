...
namespace XVSkillCreator
{
    public partial class Form1 : Form
    {
        ...
        private void buildxv2modFileToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                Title = "Save Mod File",
                Filter = "Xenoverse 2 Mod Files (*.xv2mod)|*.xv2mod"
            };
        ...
        private void openToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (ofd.ShowDialog() == DialogResult.OK)
            {
                ...
                if (!File.Exists(xmlFilePath))
                {
                    MessageBox.Show("Error: xv2mod.xml not found!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                ...
                string modType = xmlDoc.Descendants("XV2MOD").FirstOrDefault()?.Attribute("type")?.Value?.Trim() ?? "";
        ...
        {
            string datafolder = Application.StartupPath + @"/server/data";
            ...
            var kiblastSkills = cusXmlDoc.Descendants("BlastSkills").Elements("Skill").ToList();
            var awokenSkills = cusXmlDoc.Descendants("AwokenSkills").Elements("Skill").ToList();
            ...
            allSkills.AddRange(kiblastSkills.Select(skill => skill.Attribute("ShortName")?.Value ?? "Unknown"));
            allSkills.AddRange(awokenSkills.Select(skill => skill.Attribute("ShortName")?.Value ?? "Unknown"));
            ...
            XElement selectedSkill = superSkills
                ...
                ?? kiblastSkills.FirstOrDefault(skill => skill.Attribute("ShortName")?.Value == selectedSkillName)
                ?? awokenSkills.FirstOrDefault(skill => skill.Attribute("ShortName")?.Value == selectedSkillName);
